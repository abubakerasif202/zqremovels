...
const performanceStylesheetNames = {
  home: 'premium-home.min.css',
  contact: 'premium-contact.min.css',
  general: 'premium-site.min.css',
};
const purgeSafelist = {
  standard: [
    'has-error',
  ...
  deep: [/^has-/, /^is-/, /^quote-form-/, /^sticky-mobile-cta$/],
};
...
export async function runLegacyGenerator() {
  ...
  try {
    ...
    // 2. Load the source stylesheet. Route-scoped bundles are emitted after
    // generated HTML is available, so PurgeCSS can use the real page markup.
    const premiumSiteCss = await readFile(path.join(projectRoot, 'premium-site.css'), 'utf8');
    const siteScript = await readFile(path.join(projectRoot, 'site.js'), 'utf8');
    const assetVersion = createHash('sha256')
      .update(premiumSiteCss)
      .update(siteScript)
      .digest('hex')
      .slice(0, 12);

    // 3. Root-level pages are now natively generated by Astro

    // 4. Pre-load available responsive image variants
    ...
    // 6. Emit route-scoped CSS and copy static assets
    await writePerformanceStylesheets(premiumSiteCss, siteScript, renderedHtmlByOutput);
    ...
    // 8. Write LLMs/AI metadata files and robots.txt
...
// Automatically run the generator if NOT being run inside Astro/Vite
const isAstro = process.argv.some((arg) => /astro|vite/i.test(arg));
if (!isAstro) {
  await runLegacyGenerator().catch((err) => {
    console.error('Build failed:', err);
    process.exit(1);
  });
}

const homepageCriticalCss = minifyCss(`
...
@font-face{font-family:"Fraunces";font-style:normal;font-weight:500 700;font-display:swap;src:url("/fonts/fraunces-latin.woff2") format("woff2")}
:root{--premium-ink:#0a1410;--premium-paper:#12221c;--premium-cream:#050a08;--premium-orange:#c9a86a;--premium-orange-dark:#9f7d42;--premium-copy:#f7f4ed;--premium-muted:rgba(255,255,255,.68);--premium-line:rgba(201,168,106,.24);--font-heading:"Fraunces",Georgia,serif;--font-body:"Inter",sans-serif;--content-width:1280px}
*{box-sizing:border-box}html{background:var(--premium-ink);text-size-adjust:100%;scroll-behavior:smooth}body{margin:0;color:var(--premium-copy);background:linear-gradient(90deg,rgba(255,255,255,.01) 1px,transparent 1px) 0 0/4rem 4rem,var(--premium-cream);font-family:var(--font-body);line-height:1.6}.container{width:min(var(--content-width),calc(100% - clamp(2rem,6vw,6rem)));margin-inline:auto}a{color:inherit}.skip-link{position:absolute;left:1rem;top:1rem;z-index:1000;transform:translateY(-150%);background:var(--premium-paper);color:var(--premium-copy);padding:.75rem 1rem}.skip-link:focus{transform:none}.breadcrumb{display:none}.site-header{position:sticky;top:0;z-index:100;border-bottom:1px solid var(--premium-line);background:rgba(10,20,16,.94);box-shadow:0 8px 30px rgba(0,0,0,.45);backdrop-filter:blur(18px)}.utility-bar{min-height:2.15rem;color:var(--premium-copy);background:#111f1b;border-bottom:1px solid var(--premium-line)}.utility-row{display:flex;align-items:center;justify-content:space-between;gap:1rem;min-height:2.15rem;font-size:.72rem;font-weight:850;letter-spacing:.065em;text-transform:uppercase}.utility-links{display:flex;align-items:center;gap:1rem}.header-main{display:flex;align-items:center;justify-content:space-between;gap:1.25rem;min-height:5.45rem}.brand{display:inline-flex;align-items:center;gap:.7rem;color:var(--premium-copy);text-decoration:none}.brand-mark-logo{display:block;flex:0 0 3.35rem;width:3.35rem;height:3.35rem}.brand-mark-logo img{display:block;width:100%;height:100%;object-fit:contain}.brand-copy{display:grid}.brand-copy strong{color:var(--premium-copy);font-family:var(--font-heading);font-size:1.15rem;font-weight:650;letter-spacing:-.025em}.brand-copy>span{color:var(--premium-orange-dark);font-size:.68rem;font-weight:850;letter-spacing:.08em;text-transform:uppercase}.desktop-nav{display:flex;align-items:center;gap:.12rem}.desktop-nav>.nav-link,.desktop-nav>.nav-group>summary{min-height:2.75rem;padding:.72rem .7rem;border-radius:.4rem;color:var(--premium-copy);font-size:.78rem;font-weight:850;text-decoration:none}.nav-group{position:relative}.nav-group summary{list-style:none;cursor:pointer}.nav-panel{position:absolute;top:100%;left:0;min-width:18rem;padding:.65rem;border:1px solid var(--premium-line);border-radius:.75rem;background:var(--premium-paper);box-shadow:0 24px 70px rgba(0,0,0,.5)}.nav-panel-link{display:grid;gap:.15rem;padding:.8rem;border-radius:.45rem;text-decoration:none}.nav-link-title{font-weight:850}.nav-link-desc{color:var(--premium-muted);font-size:.82rem}.header-actions{display:flex;align-items:center;gap:.75rem}.header-phone{color:var(--premium-copy);font-size:.82rem;font-weight:900}.button,.btn{display:inline-flex;align-items:center;justify-content:center;min-height:3.1rem;padding:.85rem 1.25rem;border:1px solid transparent;border-radius:.4rem;font-size:.78rem;font-weight:900;letter-spacing:.055em;line-height:1.1;text-align:center;text-decoration:none;text-transform:uppercase}.button-primary,.btn-primary{border-color:#d7b36a;color:#fffdf8!important;background:linear-gradient(135deg,#0b1f1c 0%,#173f38 58%,#9f6b2e 100%);box-shadow:0 12px 28px rgba(7,23,19,.22)}.button-secondary,.btn-outline{border-color:var(--premium-line);color:var(--premium-copy);background:var(--premium-paper)}.mobile-nav{display:none}.od-hero{position:relative;overflow:hidden;padding-block:clamp(3.8rem,7vw,6rem);color:var(--premium-paper);background:radial-gradient(circle at 82% 16%,rgba(201,168,106,.12),transparent 30rem),linear-gradient(120deg,#050a08 0%,#111f1b 100%)}.od-hero-grid{display:grid;grid-template-columns:minmax(0,1.02fr) minmax(320px,.98fr);align-items:center;gap:clamp(2rem,5vw,5rem)}.od-hero-copy{display:grid;gap:1rem;max-width:46rem}.od-kicker{color:var(--premium-orange);font-size:.72rem;font-weight:900;letter-spacing:.14em;text-transform:uppercase}.od-hero .od-kicker{color:var(--premium-orange)}.od-hero h1{margin:0;color:var(--premium-paper);font-family:var(--font-heading);font-size:clamp(3.6rem,8vw,7rem);font-weight:600;letter-spacing:-.055em;line-height:.9}.od-hero-statement{margin:0;color:var(--premium-orange);font-family:var(--font-heading);font-size:clamp(2rem,4vw,3.9rem);line-height:.96}.od-hero p{margin:0;color:rgba(255,253,248,.82);max-width:42rem}.od-hero-lede{font-size:clamp(1.1rem,2vw,1.35rem)}.od-hero-actions,.od-hero-assurance,.od-intake-strip{display:flex;flex-wrap:wrap;gap:.75rem}.od-hero-assurance span,.od-intake-strip span{border:1px solid rgba(255,255,255,.18);border-radius:999px;padding:.52rem .8rem;color:rgba(255,253,248,.88);font-size:.78rem;font-weight:800}.od-proof-row{display:grid;grid-template-columns:repeat(3,1fr);margin:1rem 0 0;border:1px solid rgba(255,255,255,.18)}.od-proof-row div{padding:1rem}.od-proof-row dt{color:var(--premium-orange);font-family:var(--font-heading);font-size:1.5rem}.od-proof-row dd{margin:0;color:rgba(255,253,248,.74);font-size:.75rem}.od-hero-media{position:relative}.od-hero-media picture{display:block;aspect-ratio:768/406;overflow:hidden;border:.35rem solid var(--premium-paper);background:#102d25;box-shadow:.75rem .75rem 0 var(--premium-orange)}.od-hero-media img{display:block;width:100%;height:100%;object-fit:cover}.od-hero-note{position:absolute;right:1rem;bottom:1rem;max-width:18rem;padding:1rem;background:rgba(18,34,28,.95);color:var(--premium-copy);box-shadow:0 16px 42px rgba(0,0,0,.4);border:1px solid var(--premium-line)}.od-hero-note strong,.od-hero-note span{display:block}.od-link-panel a{display:flex;align-items:center;min-height:2.75rem}.sticky-mobile-cta{position:fixed;right:1rem;bottom:1rem;left:1rem;z-index:90;display:none;gap:.5rem}.sticky-mobile-cta.is-visible{display:flex}@media (max-width:980px){.desktop-nav,.header-actions .header-phone{display:none}.mobile-nav{display:block}.mobile-nav-top{display:flex;gap:.6rem;padding-bottom:.85rem}.mobile-menu-trigger{display:block;padding:.8rem 1rem;background:var(--premium-paper);color:var(--premium-copy);font-weight:900}.mobile-nav-panel{padding:1rem;background:var(--premium-paper)}.od-hero-grid{grid-template-columns:1fr}.od-hero-copy{max-width:100%}}@media (max-width:640px){.container{width:min(calc(100% - 1.25rem),var(--content-width))}.utility-row{align-items:flex-start;flex-direction:column;padding-block:.45rem}.brand-mark-logo{flex-basis:2.875rem;width:2.875rem;height:2.875rem}.brand-copy>span,.utility-proof{display:none}.od-hero{padding-top:3.2rem}.od-hero h1{font-size:clamp(3.35rem,17vw,4.9rem)}.od-proof-row{grid-template-columns:1fr}.od-hero-media{width:calc(100% - 1rem);max-width:100%}.od-hero-media picture{min-height:20rem}.od-hero-note{position:static;max-width:none;margin-top:.8rem}.mobile-nav-top .button{flex:1;min-height:2.9rem;padding:.76rem .65rem;font-size:.68rem}.sticky-mobile-cta .button{flex:1;min-height:2.9rem;padding:.76rem .65rem;font-size:.68rem}}
body.page-home{--zq-v2-bg:#f7f4ec;--zq-v2-surface:#fffdfa;--zq-v2-ink:#071713;--zq-v2-muted:#4e625d;--zq-v2-border:#d8ded7;--zq-v2-accent:#ff6426;--zq-v2-mint:#dfeee9;color:var(--zq-v2-ink);background:var(--zq-v2-bg);background-image:none}
body.page-home .site-header{border-color:var(--zq-v2-border);color:var(--zq-v2-ink);background:rgba(255,253,250,.97);box-shadow:0 8px 24px rgba(7,23,19,.07)}
body.page-home .utility-bar{color:#fffdfa;background:var(--zq-v2-ink)}body.page-home .utility-bar :is(span,a){color:#fffdfa}
body.page-home .header-main>.brand,body.page-home .brand-copy strong,body.page-home .brand-copy>span,body.page-home .desktop-nav>.nav-link,body.page-home .header-phone{color:var(--zq-v2-ink)}
body.page-home .header-main .button-primary,body.page-home .mobile-nav .button-primary{border-color:var(--zq-v2-accent);color:var(--zq-v2-ink)!important;background:var(--zq-v2-accent);box-shadow:none}
body.page-home .zq-v2-home{overflow:clip;color:var(--zq-v2-ink);background:var(--zq-v2-bg)}body.page-home .zq-v2-home .container{width:min(calc(100% - 40px),1220px);margin-inline:auto}
body.page-home .zq-v2-hero{position:relative;padding:clamp(2.7rem,5vw,4.4rem) 0 clamp(4rem,7vw,6rem)}body.page-home .zq-v2-hero:before{position:absolute;inset:0 0 auto;height:68%;background:var(--zq-v2-mint);content:""}
body.page-home .zq-v2-hero-grid{position:relative;display:grid;grid-template-columns:minmax(0,.88fr) minmax(34rem,1.12fr);align-items:center;gap:clamp(2rem,5vw,4.5rem)}body.page-home .zq-v2-home h1{margin:0;max-width:11ch;color:var(--zq-v2-ink);font-family:var(--font-heading);font-size:clamp(3rem,5.8vw,5.25rem);line-height:1.04;letter-spacing:-.045em}
body.page-home .zq-v2-quote-panel{overflow:hidden;border:1px solid var(--zq-v2-border);border-radius:1.35rem;background:var(--zq-v2-surface);box-shadow:0 24px 70px rgba(7,23,19,.16)}body.page-home .zq-v2-quote-media{min-height:14.5rem;overflow:hidden;background:var(--zq-v2-mint)}body.page-home .zq-v2-quote-form{padding:1.5rem;color:var(--zq-v2-ink);background:var(--zq-v2-surface)}
@media(max-width:1120px){body.page-home .desktop-nav,body.page-home .header-actions{display:none}body.page-home .mobile-nav{display:block}body.page-home .zq-v2-hero-grid{grid-template-columns:1fr}}
@media(max-width:640px){body.page-home .zq-v2-home .container{width:min(calc(100% - 24px),1220px)}body.page-home .zq-v2-home h1{font-size:clamp(2.65rem,14vw,4rem)}}
`);

export function renderHead(page, content) {
  const ogImage =
  ...
  const tags = [
    ...
    `<meta name="theme-color" content="${escapeAttribute(page.themeColor || '#0A1410')}" />`,
  ...
  if (page.layout !== 'redirect') {
    ...
    if (page.output === 'index.html') {
      ...
      tags.push(`<link rel="stylesheet" href="/${performanceStylesheetNames.home}" />`);
    } else if (page.output === 'contact-us/index.html') {
      tags.push(`<link rel="stylesheet" href="/${performanceStylesheetNames.contact}" />`);
    } else {
      tags.push(`<link rel="stylesheet" href="/${performanceStylesheetNames.general}" />`);
    }
...
async function purgeStylesheet(css, htmlDocuments, siteScript) {
  const content = [
    ...htmlDocuments.map((raw) => ({ raw, extension: 'html' })),
    { raw: siteScript, extension: 'js' },
  ];
...
async function writePerformanceStylesheets(css, siteScript, renderedHtmlByOutput) {
  const homeHtml = renderedHtmlByOutput.get('index.html');
  const contactHtml = renderedHtmlByOutput.get('contact-us/index.html');
  const generalHtml = [...renderedHtmlByOutput.entries()]
    .filter(([output]) => output !== 'index.html' && output !== 'contact-us/index.html')
    .map(([, html]) => html);

  if (!homeHtml || !contactHtml || generalHtml.length === 0) {
    throw new Error('Cannot create route-scoped CSS without generated home, contact, and general pages.');
  }

  const [homeCss, contactCss, generalCss] = await Promise.all([
    purgeStylesheet(css, [homeHtml], siteScript),
    purgeStylesheet(css, [contactHtml], siteScript),
    purgeStylesheet(css, generalHtml, siteScript),
  ]);

  const bundles = [
    [performanceStylesheetNames.home, homeCss],
    [performanceStylesheetNames.contact, contactCss],
    [performanceStylesheetNames.general, generalCss],
  ];

  for (const [filename, outputCss] of bundles) {
    await writeFile(path.join(distRoot, filename), `${outputCss}\n`, 'utf8');
    console.log(`Wrote ${filename} (${Buffer.byteLength(outputCss)} bytes).`);
  }
}

function versionPerformanceAssetReferences(html, assetVersion) {
  return html.replace(
    /\/(premium-(?:home|contact|site)\.min\.css|site\.js)(?![A-Za-z0-9?._-])/g,
    `/$1?v=${assetVersion}`,
  );
}
...
export function transformContent(content, page) {
...
function injectResponsiveSrcset(html, responsiveVariants) {
  ...
  const originalMediaWidths = new Map([
    ['about-team-branded.webp', 1200],
    ['cbd-apartment-hero.webp', 1152],
    ['coastal-suburb-hero.webp', 1152],
    ['contact-quote-branded.webp', 768],
    ['custom-operations-source.webp', 1200],
    ['furniture-wrapping-hero.webp', 1152],
    ['home-local-hero-branded.webp', 768],
    ['interstate-route-hero.webp', 960],
    ['local-move-hero.webp', 1152],
    ['north-suburb-hero.webp', 1152],
    ['office-relocation-hero.webp', 960],
    ['packing-station-hero.webp', 1152],
    ['planning-checklist-hero.webp', 1152],
    ['suburb-family-hero.webp', 1152],
    ['suburb-general-hero.webp', 960],
    ['zq-interstate-premium.webp', 1200],
    ['zq-local-premium.webp', 1200],
    ['zq-operations-premium.webp', 1200],
    ['zq-removals-social-share.webp', 1536],
    ['zq-service-premium.webp', 1200],
  ]);
  ...
  function buildSrcset(srcUrl, encodedName) {
    ...
    const originalWidth = originalMediaWidths.get(decodedName) || 1200;
    if (!parts.some((part) => part.endsWith(` ${originalWidth}w`))) {
      parts.push(`${srcUrl} ${originalWidth}w`);
    }
	    return parts.join(', ');
	  }
...
export function renderBodyTop(page, content) {
...
async function copyStaticAssets() {
  ...
  try {
    ...
    await rm(path.join(distRoot, 'chunks'), { recursive: true, force: true });
    await esbuild.build({
